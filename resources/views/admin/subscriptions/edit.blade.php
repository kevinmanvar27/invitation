                ...
                <div class="p-6 text-primary-dark">
                    ...
                    <form method="POST" action="{{ route('admin.subscriptions.update', $subscription->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" name="price" id="price" class="shadow appearance-none border rounded w-full py-2 px-3 text-primary-dark leading-tight focus:outline-none focus:shadow-outline" value="{{ old('price', $subscription->price) }}" required>
                            @error('price')
                                <p class="text-error-dark text-xs italic mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="started_at" id="started_at" class="shadow appearance-none border rounded w-full py-2 px-3 text-primary-dark leading-tight focus:outline-none focus:shadow-outline" value="{{ old('started_at', $subscription->started_at ? $subscription->started_at->format('Y-m-d') : '') }}" required>
                            @error('started_at')
                                <p class="text-error-dark text-xs italic mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="expires_at" id="expires_at" class="shadow appearance-none border rounded w-full py-2 px-3 text-primary-dark leading-tight focus:outline-none focus:shadow-outline" value="{{ old('expires_at', $subscription->expires_at ? $subscription->expires_at->format('Y-m-d') : '') }}" required>
                            @error('expires_at')
                                <p class="text-error-dark text-xs italic mt-2">{{ $message }}</p>
                            @enderror
                        </div>

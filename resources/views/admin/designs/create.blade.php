        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-primary-dark">
                    <h3 class="text-lg font-medium mb-4">Add New User Design</h3>
                    ...
                    <form method="POST" action="{{ route('admin.designs.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="design_name" id="design_name" class="shadow appearance-none border border-accent rounded w-full py-2 px-3 text-primary-dark leading-tight focus:outline-none focus:shadow-outline" value="{{ old('design_name') }}" required>
                            @error('design_name')
                                <p class="text-error-dark text-xs italic mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="block text-primary-dark text-sm font-bold mb-2">
                                <input type="checkbox" name="is_completed" id="is_completed" class="mr-2 leading-tight" {{ old('is_completed') ? 'checked' : '' }}>
                                Is Completed
                            </label>
                            @error('is_completed')
                                <p class="text-error-dark text-xs italic mt-2">{{ $message }}</p>

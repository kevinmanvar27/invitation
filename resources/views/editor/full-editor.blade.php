<x-admin-layout>
        ...
        <div class="canvas-container">
            ...
            <div class="editor-top-toolbar">
                ...
                <div class="toolbar-right">
                    ...
                    <button class="toolbar-btn primary" onclick="saveDesign(event)">
    ...
    <script>
        ...
        async function handleBackgroundImageUpload(event) {
            ...
            if (file) {
                try {
                    ...
                    const response = await fetch('{{ route("admin.designs.upload-image") }}', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    if (result.success) {
                        canvasBackground.image = result.url;
                        ...
                        alert('Failed to upload background image: ' + result.message);
                    }
                } catch (error) {
            ...
            list.innerHTML = [...elements].reverse().map((el) => {
                ...
                switch(el.type) {
                    ...
                    case 'image':
                        icon = 'fa-image';
                        name = 'Image';
                        thumbnail = el.src ? `<img src="${el.src}" alt="">` : '';
                        break;
                    case 'shape':
                        icon = 'fa-shapes';
                        name = el.shapeType ? (el.shapeType.charAt(0).toUpperCase() + el.shapeType.slice(1)) : 'Shape';
                        break;
                    case 'line':
                        icon = 'fa-minus';
                        name = 'Line';
            ...
            for (const file of files) {
                if (file.type.startsWith('image/')) {
                    ...
                    if (grid) {
                        ...
                        loadingDiv.innerHTML = '<div style="text-align: center; padding: 20px;">Uploading...</div>';
        ...
        // Helper function to check for base64 images in canvas data
        ...
        // Convert all base64 images in canvas data to uploaded URLs
        async function convertBase64ImagesToUrls(canvasData) {
            let converted = 0;
            
            // Process all pages
            for (let page of canvasData.pages) {
                // Process page elements
                if (page.elements) {
                    for (let element of page.elements) {
                        // Check if element is an image with base64 src
                        if (element.type === 'image' && element.src && element.src.startsWith('data:image')) {
                            try {
                                console.log('Converting base64 image to URL for element:', element.id);
                                
                                // Extract mime type
                                const mimeMatch = element.src.match(/data:(image\/[^;]+);base64,/);
                                const mimeType = mimeMatch ? mimeMatch[1] : 'image/png';
                                const extension = mimeType.split('/')[1];
                                
                                // Convert to blob
                                const blob = base64ToBlob(element.src, mimeType);
                                
                                // Create file from blob
                                const file = new File([blob], `image_${element.id}.${extension}`, { type: mimeType });
                                ...
                                if (result.success) {
                                    ...
                                    converted++;
                                    console.log('Successfully converted image:', element.id, 'to URL:', result.url);
                                } else {
                ...
                if (page.background && page.background.image && page.background.image.startsWith('data:image')) {
                    try {
                        console.log('Converting base64 background image to URL for page:', page.id);
                        
                        const mimeMatch = page.background.image.match(/data:(image\/[^;]+);base64,/);
                        const mimeType = mimeMatch ? mimeMatch[1] : 'image/png';
                        const extension = mimeType.split('/')[1];
                        
                        const blob = base64ToBlob(page.background.image, mimeType);
                        const file = new File([blob], `background_${page.id}.${extension}`, { type: mimeType });
                        ...
                        if (result.success) {
                            page.background.image = result.url;
                            converted++;
                            console.log('Successfully converted background image to URL:', result.url);
                        }
                    } catch (error) {
            ...
            // Process canvas-level background
            if (canvasData.background && canvasData.background.image && canvasData.background.image.startsWith('data:image')) {
                try {
                    console.log('Converting base64 canvas background image to URL');
                    
                    const mimeMatch = canvasData.background.image.match(/data:(image\/[^;]+);base64,/);
                    const mimeType = mimeMatch ? mimeMatch[1] : 'image/png';
                    const extension = mimeType.split('/')[1];
                    
                    const blob = base64ToBlob(canvasData.background.image, mimeType);
                    const file = new File([blob], `canvas_background.${extension}`, { type: mimeType });
                    ...
                    if (result.success) {
                        canvasData.background.image = result.url;
                        converted++;
                        console.log('Successfully converted canvas background image to URL:', result.url);
                    }
                } catch (error) {
                    console.error('Error converting canvas background image:', error);
                }
            }
        ...
        async function saveDesign(event) {
            ...
            // Check for base64 images and convert them automatically
            const base64Count = checkForBase64Images(canvasData);
            if (base64Count > 0) {
                console.warn(`Warning: Found ${base64Count} base64-encoded images. Converting to uploaded images...`);
                
                if (!confirm(`Found ${base64Count} embedded image(s). Would you like to automatically convert them to uploaded images? This may take a moment.\n\nClick OK to convert, or Cancel to abort save.`)) {
                    return;
                }
                
                // Show loading indicator
                const originalText = event.target ? event.target.textContent : 'Save';
                if (event.target) {
                    event.target.textContent = 'Converting images...';
                    event.target.disabled = true;
                }
                
                try {
                    const converted = await convertBase64ImagesToUrls(canvasData);
                    console.log(`Successfully converted ${converted} images to URLs`);
                    
                    // Update the current page and elements with converted data
                    if (canvasData.pages[currentPageIndex]) {
                        pages[currentPageIndex] = canvasData.pages[currentPageIndex];
                        elements = canvasData.pages[currentPageIndex].elements;
                    }
                    
                    // Re-render to show updated images
                    renderAllElements();
                    
                    alert(`Successfully converted ${converted} image(s). Now saving...`);
                } catch (error) {
                    console.error('Error converting images:', error);
                    alert('Failed to convert some images. Please try again or upload images manually.');
                    if (event.target) {
            ...
            console.log('Saving design with canvas data:', canvasData);
            console.log('Pages:', pages);
            console.log('Elements in first page:', pages[0]?.elements);
            console.log('📄 DETAILED ELEMENTS BEING SAVED:', JSON.stringify(pages[0]?.elements, null, 2));
            
            const canvasDataStr = JSON.stringify(canvasData);
            console.log('Canvas data size:', (canvasDataStr.length / 1024).toFixed(2) + ' KB');
            
            document.getElementById('canvas_data').value = canvasDataStr;
            ...
            .then(data => {
                ...
                if (data && data.success) {
                    ...
                    // Don't redirect, just reload the page to show updated data
            ...
            .catch(error => {
                ...
                alert('Error saving design. Please check console for details.');
            });
        }
